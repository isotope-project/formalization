...
structure Comp (T: Type u) extends Var T where
  cen: Bool

instance Comp.instHasLin {T} [HasLin T]: HasLin (Var T) where
  aff v := v.aff && HasLin.aff v.ty
  rel v := v.rel && HasLin.rel v.ty

instance Comp.instPartialOrder {T}: PartialOrder (Comp T) where
  le l r := l.toVar ≤ r.toVar ∧ l.cen ≤ r.cen
  le_refl := by simp [le_refl]
  le_trans _ _ _  | ⟨He, Ht⟩, ⟨He', Ht'⟩ => ⟨le_trans He He', le_trans Ht Ht'⟩
  le_antisymm x x' H H'
...
def Ctx.split.subctx_left {T: Type u} [HasLin T] {Γ Δ Ξ: Ctx T}
  ...
  | nil => subctx.nil
  | right _ _ _ H | discard _ _ H => subctx.discard _ (subctx_left H)
  | left _ _ Hl H | dup _ _ _ _ Hl _ H => subctx.cons Hl (subctx_left H)

def Ctx.split.subctx_right {T: Type u} [HasLin T] {Γ Δ Ξ: Ctx T}
  : Γ.split Δ Ξ -> Γ.subctx Ξ
  | nil => subctx.nil
  | left _ _ _ H | discard _ _ H => subctx.discard _ (subctx_right H)
  | right _ _ Hr H | dup _ _ _ _ _ Hr H => subctx.cons Hr (subctx_right H)
