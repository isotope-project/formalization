...
extends SymmetricPremonoidalCategory C where
  ...
  transparency: Linearity -> SymmetricPremonoidalSubcategory C
  drop: ∀{A}, linearity affine A -> (A ⟶ O.tensorUnit)
  drop_pure: ∀{A H q}, (transparency q).contains A O.tensorUnit (drop H)
  split: ∀{A}, linearity relevant A -> (A ⟶ A ⊗ A)
  split_pure: ∀{A H q}, (transparency q).contains A (A ⊗ A) (split H)
  split_comm: ∀{A H}, split H ≫ (braiding A A).hom = split H
  split_duplicate_left: ∀{A B HA HB f},
    (transparency relevant).contains A B f -> f ≫ split HB = split HA ≫ f ⋉ f
  split_duplicate_right: ∀{A B HA HB f},
    (transparency relevant).contains A B f -> f ≫ split HB = split HA ≫ f ⋊ f
  drop_affine: ∀{A B HA HB f},
    (transparency affine).contains A B f -> f ≫ drop HB = drop HA
